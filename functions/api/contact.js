...
// Optional CONTACT_TO_EMAIL defaults to contact@mvt-solutions.fr.
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!contentType.toLowerCase().includes('application/json')) {
      return reply(415, { ok: false, message: 'Format de demande invalide.' });
    }
    if (Number(request.headers.get('content-length') || 0) > MAX_BYTES) {
      return reply(413, { ok: false, message: 'Demande trop volumineuse.' });
    }
    ...
    try {
      ...
      return reply(400, { ok: false, message: 'Demande illisible.' });
    }

    if (!data || typeof data !== 'object' || Array.isArray(data)) {
      return reply(400, { ok: false, message: 'Demande invalide.' });
    }
    ...
    if (/\r|\n/.test(email)) {
      return reply(400, { ok: false, message: 'Adresse e-mail invalide.' });
    }
    ...
    try {
      ...
      return reply(502, { ok: false, message: 'Le service e-mail est momentanément indisponible.' });
    }
